    ...
    def set_form(defaults):
        ...
        for param in list(defaults.values()):
            ...
            if getattr(param, "inflatable", False):
                ...
                widgets[field.id] = forms.fields.CheckboxInput(attrs=attrs)
                update_fields[field.id] = forms.BooleanField(
                    label=field.label,
                    widget=widgets[field.id],
                    required=False,
                    disabled=param.gray_out,
                    initial=field.default_value == 'True'
                )
...
class TaxcalcForm(FormMixin, ModelForm):
    ...
    def __init__(self, first_year, use_puf_not_cps, *args, **kwargs):
        ...
        # time a new `TaxcalcForm` instance is created
        ...
        super(TaxcalcForm, self).__init__(*args, **kwargs)
    ...
    def set_form_data(self, start_year, use_puf_not_cps):
        ...
        (self.widgets, self.labels,
            self.update_fields) = FormMixin.set_form(defaults)
    ...
    class Meta:
        ...
        (widgets, labels,
            update_fields) = FormMixin.set_form(
