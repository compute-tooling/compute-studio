...
from .models import TaxcalcRun, TaxcalcInputs
...
def process_reform(request, compute, user=None, **kwargs):
    """
    Submits taxcalc reforms.  This handles data from the GUI interface
    and the file input interface.
    ...
    - post_meta: namedtuple containing data relevant to the parameter
        submission
    """
    post_meta = submit_reform(request, compute, user=user, **kwargs)
...
def save_model(post_meta):
    ...
    if post_meta.url is None:
        unique_url = TaxcalcRun()
    else:
        unique_url = post_meta.url
    unique_url.job_id = post_meta.submitted_id
    unique_url.inputs = model
    print('profile', post_meta.user.profile)
    unique_url.profile = post_meta.user.profile
    unique_url.project = Project.objects.get(name='Tax-Calculator')
...
def submit_reform(request, compute, user=None, inputs_id=None):
    """
    ...
    use_full_sample = False if fields.get('quick_calc') else True
    if use_full_sample and 'full_calc' in fields:
    ...
    if inputs_id:
        try:
            model = TaxcalcInputs.objects.get(id=int(inputs_id))
        except Exception:
        ...
        if 'docfile' in request_files:
            ...
            personal_inputs = TaxcalcForm(start_year, use_puf_not_cps, fields)
    ...
    years_n = (list(range(NUM_BUDGET_YEARS)) if use_full_sample
    ...
    if stop_submission:
        ...
        submitted_id, max_q_length = (
            compute.submit_job(data_list, 'taxcalc'))
