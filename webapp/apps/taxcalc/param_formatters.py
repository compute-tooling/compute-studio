...
def get_default_policy_param(param, default_params):
    """
    Map webapp field name to Tax-Calculator parameter name
...
def get_reform_from_gui(start_year, taxcalc_fields=None, behavior_fields=None,
    ...
    returns reform and assumptions dictionaries that are compatible with
            ...
            if necessary
    """
    if taxcalc_fields is None and behavior_fields is None:
        raise ValueError("Neither taxcalc data nor behavior data was given")

    reform_inputs = {}
    assumption_inputs = {}

    # prepare taxcalc params from TaxSaveInputs model
    if taxcalc_fields is not None:
