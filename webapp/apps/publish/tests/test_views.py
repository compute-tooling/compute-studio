...
from webapp.apps.users.models import Project, Visualization, Profile

from webapp.apps.users.serializers import ProjectSerializer, VisualizationSerializer
...
class TestPublishViews:
    ...
    def test_post(self, client):
        ...
        viz_data = {
            ...
            "software": "dash",
            "function_name": "dash_server_func",
            "requires_server": False,
        }

        resp = client.post(
            f"/publish/api/{project.owner}/{project.title}/viz/", viz_data
        )
        assert resp.status_code == 200, resp.data

        viz = Visualization.objects.get(project=project, title=viz_data["title"])
        assert viz.has_write_access(project.owner.user)
        assert viz.has_write_access(api_user.user)
    ...
    def test_get_detail_api(self, api_client, client, test_models):
        ...
        assert serializer.validated_data == {**exp, **{"visualizations": []}}
        ...
        viz = Visualization.objects.create(**dict(project=project, **viz_data))
        api_client.logout()
        resp = api_client.get("/publish/api/modeler/Detail-Test/viz/My-Viz/")
        assert resp.status_code == 200
        data = resp.json()

        serializer = VisualizationSerializer(viz, data=data)
        assert serializer.is_valid(), serializer.errors
        assert serializer.validated_data == dict(is_live=False, **viz_data)
    ...
    def test_put_detail_api(self, client, test_models, profile, password):
        ...
        resp = client.put(
            "/publish/api/modeler/Used-for-testing/viz/Test-Viz/",
            data=viz_data,
            content_type="application/json",
        )
        assert resp.status_code == 200, resp.data
        viz = Visualization.objects.get(
            project__title="Used-for-testing",
            project__owner__user__username="modeler",
            title="Test-Viz",
        )
        assert viz.description == "new"
        assert viz.function_name == "hello_world"

        remove_perm("write_project", profile.user, project)
        assert not profile.user.has_perm("write_project", project)
        assert not project.has_write_access(profile.user)
        assert not viz.has_write_access(profile.user)
